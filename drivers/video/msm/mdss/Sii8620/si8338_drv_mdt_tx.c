...
uint8_t sii8338_irq_for_mdt(uint8_t *mdt_buf) 
{
	...
	switch (mdt_state) {
		...
		case WAIT_FOR_WRITE_BURST_COMPLETE:
			//printk(KERN_ERR "WAIT_FOR_WRITE_BURST_COMPLETE\A1\A3\A1\A3,g_mdt.is_ready=0x%x...\n",g_mdt.is_ready);
			...
			break;
		case WAIT_FOR_WRITE_BURST_COMPLETE_FROM_SPIDER:
			//printk(KERN_ERR "WAIT_FOR_WRITE_BURST_COMPLETE_FROM_SPIDER\A1\A3\A1\A3\A1\A3\n");
			...
			break;								
		default:	
			printk(KERN_ERR "default\A1\A3\A1\A3\n");//... for performance, better to put
			return 0;								//    these conditions at the end of switch
