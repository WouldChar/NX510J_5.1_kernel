...
#endif
#ifndef	HALLADJ_HW
uint32_t	GinMes( unsigned char ) ;					// Measure Result Getting
#endif
void			GyrCon( unsigned char ) ;					// Gyro Filter Control
short			GenMes( unsigned short, unsigned char ) ;	// General Measure
#ifndef	HALLADJ_HW
// uint32_t	TnePtp( unsigned char, unsigned char ) ;	// Get Hall Peak to Peak Values
// unsigned char	TneCen( unsigned char, UnDwdVal ) ;			// Tuning Hall Center
 uint32_t	TneOff( UnDwdVal, unsigned char ) ;			// Hall Offset Tuning
 uint32_t	TneBia( UnDwdVal, unsigned char ) ;			// Hall Bias Tuning
#endif
...
// Measure Mode
...
#ifdef	CORRECT_04DEG
  #define		MAXLMT		0x3FD9999A				// 1.7
  #define		MINLMT		0x3F6F5C29				// 0.935
  #define		CHGCOEF		0xBA7085DB				// -0.000918
  #define		MINLMT_MOV	0x00000000				// 0.0
  #define		CHGCOEF_MOV	0xB98D2D2D				// -0.000269
 #endif	//CORRECT_04DEG
 #ifdef	CORRECT_05DEG
 ...
 #ifdef	CORRECT_07DEG
  #define		MAXLMT		0x40333333				// 2.8
  #define		MINLMT		0x3FC51EB9				// 1.54
  #define		CHGCOEF		0xBA120820				// -0.00056
  #define		MINLMT_MOV	0x00000000				// 0.0
  #define		CHGCOEF_MOV	0xB92B6DB7				// -0.00016
 #endif	//CORRECT_07DEG
 
 #ifdef	CORRECT_12DEG
  #define		MAXLMT		0x405B6DB7				// 3.429
  #define		MINLMT		0x3FF15F16				// 1.886
  #define		CHGCOEF		0xB9EE84BD				// -0.00045
  #define		MINLMT_MOV	0x00000000				// 0.0
  #define		CHGCOEF_MOV	0xB90C0000				// -0.00013
 #endif	//CORRECT_12DEG
#endif
...
#define		ZOOMTBL	16
const uint32_t	ClGyxZom[ ZOOMTBL ]	= {
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
...
const uint32_t	ClGyyZom[ ZOOMTBL ]	= {
		0x3F800000,
		0x3F800000,
		0x3F800000,
		0x3F800000,
...
//	UcHlxSts	= TneCen( X_DIR, StTneVal ) ;
	...
	StAdjPar.StHalAdj.UsAdxOff = (unsigned short)((uint32_t)0x00010000 - (uint32_t)StAdjPar.StHalAdj.UsHlxCna ) ;
	StAdjPar.StHalAdj.UsAdyOff = (unsigned short)((uint32_t)0x00010000 - (uint32_t)StAdjPar.StHalAdj.UsHlyCna ) ;
...
uint32_t	TnePtp ( unsigned char	UcDirSel, unsigned char	UcBfrAft )
{
	UnDwdVal		StTneVal ;

	MesFil( THROUGH ) ;					// \91\AA\92\E8\97p\83t\83B\83\8B\83^\81[\82\F0\90ݒ肷\82\E9\81B
...
// History			: First edition 						2009.12.1 YS.Kim
//********************************************************************************
uint32_t	TneBia( UnDwdVal	StTneVal, unsigned char	UcTneAxs )
{
	int32_t					SlSetBia ;
	...
	if( SlSetBia > ( int32_t )0x00007FFF ) {
		SlSetBia	= 0x00007FFF ;
	} else if( SlSetBia < ( int32_t )0xFFFF8001 ) {
		SlSetBia	= 0xFFFF8001 ;
	}
...
// History			: First edition 						2009.12.1 YS.Kim
//********************************************************************************
uint32_t	TneOff( UnDwdVal	StTneVal, unsigned char	UcTneAxs )
{
	int32_t			SlSetOff ;
	...
	if( SlSetOff > ( int32_t )0x00007FFF ) {
		SlSetOff	= 0x00007FFF ;
	} else if( SlSetOff < ( int32_t )0xFFFF8001 ) {
		SlSetOff	= 0xFFFF8001 ;
	}
...
//		UsFreqVal	=	0x30EE ;				/* 139.9Hz */
   ...
   #endif	//ACTREG_6P5OHM
   #ifdef	ACTREG_10P5OHM
		UsFreqVal	=	0x2D70 ;				/* 129.9Hz */
   #endif	//ACTREG_10P5OHM
   #ifdef	ACTREG_15OHM
...
//			RamWrite32A( sysin, 0x3BB84480 ) ;				// 0x11D5		-45dB
		}
...
// History			: First edition 						2013.01.15 Y.Shigeoka
//********************************************************************************
uint32_t	GinMes( unsigned char	UcXg1Xg2 )
{
	uint32_t	UlMesVal ;
...
unsigned char	TneGvc( void )
{
	...
	ClrGyr( 0x1000 , CLR_FRAM1 );					// Measure Filter RAM Clear
	StAdjPar.StGvcOff.UsGxoVal = (unsigned short)GenMes( AD2Z, 0 );		// 64\89\F1\82̕\BD\8Bϒl\91\AA\92\E8	GYRMON1(0x1110) <- GXADZ(0x144A)
	...
	RegWriteA( WC_MES1ADD1, 0x00 ) ;		// 0x0195
	ClrGyr( 0x1000 , CLR_FRAM1 );					// Measure Filter RAM Clear
	StAdjPar.StGvcOff.UsGyoVal = (unsigned short)GenMes( AD3Z, 0 );		// 64\89\F1\82̕\BD\8Bϒl\91\AA\92\E8	GYRMON2(0x1111) <- GYADZ(0x14CA)
...
/* \90U\95\9D\82\CDsxsin(0x10D5),sysin(0x11D5)\82Œ\B2\90\AE */
...
void SetSineWave( unsigned char UcJikuSel , unsigned char UcMeasMode )
{
 ...
 #endif
	uint32_t   UlAMP[2][2]   = {{ 0x3CA3D70A , 0x3CA3D70A } ,		// Loop Gain   { X amp , Y amp }
...
unsigned char  BiasOffsetAdj( unsigned char UcJikuSel , unsigned char UcMeasCnt )
{
	...
	uint32_t  UlTgtVal[2][5]    =  {{ 0x3F800000 , 0x3D200140 , 0xBD200140 , 0x3F547AE1 , 0x3F451EB8 },	/* ROUGH */
...
//********************************************************************************
void	GyrGan( unsigned char UcGygmode , uint32_t UlGygXval , uint32_t UlGygYval )
{
	switch ( UcGygmode ) {
		case VAL_SET :
...
void	SetGcf( unsigned char	UcSetNum )
{
	...
	if(UcSetNum > (COEFTBL - 1))
		UcSetNum = (COEFTBL -1) ;			/* \8F\E3\8C\C0\82\F0COEFTBL-1\82ɐݒ肷\82\E9 */
