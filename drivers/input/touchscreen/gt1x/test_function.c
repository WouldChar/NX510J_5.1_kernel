...
#ifdef CONFIG_ZTEMT_GT1X_TOUCHSCREEN_SENSOR_1
#include "test_params_sensor_1.h"
#else
#include "test_params_sensor_2.h"
#endif
#endif
...
static unsigned char _check_full_screen_jitter(void)
{
	...
	/* if 6sigama>jitter_limit or max_jitter>jitter_limit+10\A3\ACjitter is not legal */
		...
		if (test_error_code & _GT_SHORT) {
			...
			for (index = 0; index < shortresult[0]; index++) {
				...
				if (shortresult[2 + index * 4] & 0x80) {
					...
					bytes += (s32) sprintf((char *)&data[bytes], "Drv%d ֮\BC\E4\B6\CC·", shortresult[2 + index * 4] & 0x7F);
					...
					if (shortresult[2 + index * 4] == (sys.max_driver_num + 1)) {
						...
						bytes += (s32) sprintf((char *)&data[bytes], "GND\\VDD ֮\BC\E4\B6\CC·");

					}

					else {

						bytes += (s32) sprintf((char *)&data[bytes], "Sen%d ֮\BC\E4\B6\CC·", shortresult[2 + index * 4] & 0x7F);
