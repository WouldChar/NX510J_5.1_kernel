...
/****************************************************************  
	Pitter Liao add for macro for the global platform
		...
		mobile: 13244776877
-----------------------------------------------------------------*/
#define PLUG_CAL_VERSION 0x0001
/*----------------------------------------------------------------
fixed some bugs
0.1
...
struct sc_config {	
	struct sc_condition cond[NUM_SC_CONDITION];
...
static int rc_handle_sc_message(struct plugin_cal *p, const struct samples *s)
{
	...
	for (i = 0; i < (s->command.num >> 1); i++) {
		if (s->buf[i] <= sc_cfg->cond[SC_NEG].thld) {
			count[SC_NEG]++;
			count[SC_ACCU] += s->buf[i];
		}else if(s->buf[i] <= sc_cfg->cond[SC_NO].thld)
			count[SC_NO]++;
		else if(s->buf[i] > sc_cfg->cond[SC_POS].thld) {
			count[SC_ACCU] += s->buf[i];
			count[SC_POS]++;
		}
	}

	if (count[SC_POS] >= sc_cfg->cond[SC_POS].num) {
		if (sc_obs->cnt.bad)
			sc_obs->cnt.bad = 0;
		if (sc_obs->cnt.pend)
			sc_obs->cnt.pend = 0;
	}else {
		if (count[SC_ACCU] <= sc_cfg->cond[SC_ACCU].thld) {
			 if(count[SC_NEG] > sc_cfg->cond[SC_NEG].num)
				sc_obs->cnt.bad++;
			else	if (count[SC_NO] >= sc_cfg->cond[SC_NO].num) {
			...
			if(count[SC_NEG] >= sc_cfg->cond[SC_NEG].num) {
				...
				sc_obs->cnt.pend = sc_obs->cnt.bad;
				 sc_obs->cnt.bad = 0;
			}	
		}
	...
	if (ret) {
		dev_info2(dev, "rc_handle_sc_message sc(%ld) cnt (accu %d neg %d nor %d pos %d) bad %d pend %d sleep %d\n", 
			s->type, count[SC_ACCU], count[SC_NEG],	count[SC_NO],count[SC_POS],
			sc_obs->cnt.bad, sc_obs->cnt.pend,sc_obs->cnt.sleep);
...
static int rc_cal_handler(struct plugin_cal *p, const struct samples *s)
{
	...
	if (se_obs->cnt.bad > se_cfg->count.bad)
		...
		((sc_obs->cnt.pend + sc_obs->cnt.bad) > sc_cfg->count.pend)) {
...
static long plugin_cal_post_process_messages(struct plugin_cal *p, unsigned long pl_flag)
{
	...
	if (s) {
		dev_dbg(dev, "mxt s type %lx cnt %d\n", s->type, s->cnt);
		WARN_ON(s->type >= NUM_OBJ_TYPE);
		WARN_ON(!s->cnt);
		ret = mxt_surface_page_aquire_byte(pl->dev, &s->command, (char *)s->buf, 1, 0);
		if (ret == -EAGAIN)
			;
		else if(ret == 0) {
			//print_dec16_buf(KERN_INFO, "s content", s->buf, s->command.num >> 1);
...
static ssize_t rc_sc_show(struct plugin_cal *p, char *buf, size_t count)
{
	...
	if (count > 0) {
		for (i = SC_NEG; i < NUM_SC_CONDITION; i++) {
			offset += scnprintf(buf + offset, count - offset, "SC[%d]: thld %d num %d\n",
				i, sc_cfg->cond[i].thld, sc_cfg->cond[i].num);
		}
		...
		if (count > 4 && count < sizeof(name)) {
			...
			if (ret == 1) {
				if (strncmp(name, "rc", 2) == 0) {
					...
					if(sscanf(buf + offset, "cond[%d]: %d %d%n", &config[0],&config[1],&config[2],&ofs) == 3) {
						offset += ofs;
						if (config[0] >= 0 && config[0] < NUM_SC_CONDITION) {
							sc_cfg->cond[config[0]].thld = config[1];
							sc_cfg->cond[config[0]].num = config[2];
						}
...
static struct sc_condition sc_rc_condition[NUM_SC_CONDITION] = {
	{-15, 5},
	{10, 10},
	{100, 1},
	{-150,0},
...
static int init_rc_object(struct plugin_cal *p)
{
	...
	cfg->interval_next[SC_PROX] = HZ * 3 / 2;
	cfg->interval_next[SC_TOUCH] = HZ * 3 / 2;
	cfg->interval_next[SC_HOVER] = HZ * 3 / 2;

	cfg->interval_ticks[TICKS_FAST] = HZ / 8;
