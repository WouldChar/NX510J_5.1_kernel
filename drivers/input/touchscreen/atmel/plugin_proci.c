...
*/
	...
	{.reg = MXT_GEN_ACQUISITIONCONFIG_T8, .instance = 0, 
		.offset = 2,.buf = {2}, .len = 1, .mask = 0x0,.flag = BIT_MASK(P_COMMON)},

	{.reg = MXT_GEN_ACQUISITIONCONFIG_T8, .instance = 0, 
		.offset = 4,.buf = {10}, .len = 1, .mask = 0x0,.flag = BIT_MASK(P_COMMON)},

	{.reg = MXT_GEN_ACQUISITIONCONFIG_T8, .instance = 0, 
		.offset = 11,.buf = {2}, .len = 1, .mask = 0x0,.flag = BIT_MASK(P_COMMON)},

	{.reg = MXT_GEN_ACQUISITIONCONFIG_T8, .instance = 0, 
		.offset = 14,.buf = {0x1}, .len = 1, .mask = 0x0,.flag = BIT_MASK(P_COMMON)},

	{.reg = MXT_SPT_GPIOPWM_T19,
		.offset = 0,.buf = {1,0,0,15}, .len = 4, .mask = 0,.flag = BIT_MASK(P_COMMON)},

#if defined(CONFIG_DISABLE_TIMER_AT_SUSPEND)
	/*{.reg = MXT_SPT_TIMER_T61, .instance = 0, 
		.offset = 0,.buf = {0}, .len = 1, .mask = 0x1,.flag = BIT_MASK(P_COMMON)}, */
	{.reg = MXT_SPT_TIMER_T61, .instance = 1, 
		.offset = 0,.buf = {0}, .len = 1, .mask = 0x1,.flag = BIT_MASK(P_COMMON)},
	{.reg = MXT_SPT_TIMER_T61, .instance = 2, 
		.offset = 0,.buf = {0}, .len = 1, .mask = 0x1,.flag = BIT_MASK(P_COMMON)},
	{.reg = MXT_SPT_TIMER_T61, .instance = 3, 
		.offset = 0,.buf = {0}, .len = 1, .mask = 0x1,.flag = BIT_MASK(P_COMMON)},
	{.reg = MXT_SPT_TIMER_T61, .instance = 4, 
		.offset = 0,.buf = {0}, .len = 1, .mask = 0x1,.flag = BIT_MASK(P_COMMON)},
	{.reg = MXT_SPT_TIMER_T61, .instance = 5, 
		.offset = 0,.buf = {0}, .len = 1, .mask = 0x1,.flag = BIT_MASK(P_COMMON)},
#endif
...
*/
	...
	{.reg = MXT_PROCI_TOUCHSUPPRESSION_T42,
		.offset = 1,.buf = {0X0,0x32,0x19,0x80,0x0,0x0,0x0,0x0,0x0,0xFF}, .len = 0, .mask = 0,.flag = BIT_MASK(P_COMMON)},
...
ssize_t zte_plugin_proci_pi_gesture_show(struct plugin_proci *p, char *buf, size_t count)
{
	struct pi_config *cfg = p->cfg;
...
//const struct ges_tab_element *elem;
	...
	list_for_each(node,head) {
        ...
        if (val == 1)
            {
                //dev_err(dev, "zte_plugin_proci_pi_gesture_show need en_gesture!\n");
                return 1;
            }
		}
	return 0;
...
int plugin_proci_pi_gesture_store(struct plugin_proci *p, const char *buf, size_t count)
{
	...
	do {
		...
		//dev_info(dev, "Get %s=%x; len %d ofs %d count %ud\n",name,val,len,ofs,count);
		len = strlen(name);
		op = 0;
		if (val & (1<<0))
			op = BIT_MASK(OP_SET);
...
static int plugin_proci_pi_show(struct plugin_proci *p)
{
	...
	for (i = 0; i < PI_LIST_NUM; i++) {
		...
		for (j = 0; j < cfg->num_reg_cfg[i]; j++) {
			dev_info(dev, "[mxt]config %d-%d: T%d(%d) offset %d len %d(%lx,%lx,%hhd): ",i,j, r[j].reg,r[j].instance, r[j].offset,r[j].len,r[j].mask,r[j].flag,r[j].sleep);
					...
					if (ret == 2) {
						...
						if (i >= 0 && i < PI_LIST_NUM) {
							if (j >= 0 && j < cfg->num_reg_cfg[i]) {
								...
								ofs = 0;
								ret = sscanf(buf + offset, "T%hd(%hd) offset %hd len %hd(%lx,%lx,%hhd): %n", 
									&rc.reg, &rc.instance, &rc.offset, &rc.len, &rc.mask, &rc.flag, &rc.sleep, &ofs);
								if (ret > 0) {
									dev_info(dev, "%s\n",buf + offset);
									dev_info(dev, "T%hd(%d) offset %hd len %hd(%lx,%lx):(ret %d ofs %d)^",rc.reg, rc.instance, rc.offset,rc.len, rc.mask, rc.flag, ret, ofs);
		...
		if (ret == 0) {
				...
				if (sym_cnt) {
					if (sym_tag >= CHARACTER_ASCII_BEGIN) {
						...
						cont = (struct data_obj *)create_new_node(&gobj->sublist,sizeof(struct data_obj));
						if (!cont) {
...
static int plugin_proci_init(struct plugin_proci *p)
{
	const struct mxt_config *dcfg = p->dcfg;
...
static void plugin_proci_deinit(struct plugin_proci *p)
{
	deinit_pi_object(p);
...
static struct plugin_proci mxt_plugin_proci_pi_if = 
{
	.init = plugin_proci_init,
	.deinit = plugin_proci_deinit,
	.start = NULL,
	.stop = NULL,
...
int plugin_interface_proci_init(struct plugin_proci *p)
{
	memcpy(p, &mxt_plugin_proci_pi_if, sizeof(struct plugin_proci));
